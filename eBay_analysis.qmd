...
| Question | Answer |
|:---------|:-------|
| **Most competitive category?** | `{python} category_competitive['mean'].idxmax()` |
| **Most competitive day?** | `{python} endday_competitive['mean'].idxmax()` |
| **% of auctions competitive?** | `{python} f"{competitive_percentage:.2f}%"` |
| **Imbalance problem?** | `{python} "Yes" if (competitive_percentage < 40 or competitive_percentage > 60) else "No"` |
| **Training accuracy (with ClosePrice)?** | `{python} f"{train_accuracy_with_close*100:.2f}%"` |
| **Validation accuracy (with ClosePrice)?** | `{python} f"{val_accuracy_with_close*100:.2f}%"` |
| **Training accuracy (without ClosePrice)?** | `{python} f"{train_accuracy_without_close*100:.2f}%"` |
| **Validation accuracy (without ClosePrice)?** | `{python} f"{val_accuracy_without_close*100:.2f}%"` |
| **Accuracy change after removing ClosePrice?** | `{python} f"{val_accuracy_change*100:+.2f}%"` |
